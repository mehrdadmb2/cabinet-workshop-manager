...
(() => {
  ...
  const state = {
    view:'dashboard', project:null, projects:[], sheetIndex:0, sheetSearch:'', cellSearch:'', selectedCell:null,
  ...
  function renderDashboard(){
    ...
    const sheets=p?.workbook?.sheets?.length||0, drawings=p?.drawings?.length||0, parts=p?.parts?.length||0, payments=p?.payments?.length||0;
    const recent=(state.projects||[]).filter(x=>x.id!==p?.id).slice(0,3);
    if(!p)return `<div class="hero dashboard-hero"><div><span class="eyebrow">CABINET WORKSHOP OS</span><h1>کارگاه را ساده‌تر اداره کن؛<br><em>همه‌چیز یکجا، بدون دردسر Excel.</em></h1><p>برای شروع، پروژه جدید بساز یا فایل Excel قدیمی را وارد کن. بعد از آن، طراحی، نقشه، قطعات، پرداخت و خروجی‌ها را داخل همین برنامه ادامه بده.</p><div class="hero-actions"><button class="btn primary xl" data-action="new">＋ پروژه جدید</button><button class="btn glass xl" data-action="import">⇧ ورود Excel</button><button class="btn glass xl" data-action="open-project">↥ باز کردن پروژه</button></div><div class="command-strip"><span class="muted">مسیر سریع</span><span class="command-chip">۱. پروژه</span><span class="command-chip">۲. نقشه</span><span class="command-chip">۳. قطعات</span><span class="command-chip">۴. خروجی</span></div></div><div class="hero-card"><div class="hero-card-top"><span class="badge cyan">READY</span><span class="muted">آماده شروع</span></div><div class="metric"><strong>۰</strong><span>پروژه فعال</span></div><div class="metric"><strong>۰</strong><span>نقشه</span></div><div class="metric"><strong>۰</strong><span>قطعه</span></div><div class="metric"><strong>۰</strong><span>پرداخت</span></div></div></div><div class="dashboard-grid"><article class="panel dashboard-panel"><div class="panel-head"><div><span class="eyebrow">START HERE</span><h2>سه راه ساده برای شروع</h2></div></div><div class="quick-actions dashboard-actions"><button class="action-card" data-action="new"><b>＋</b><span>پروژه جدید</span><small>شروع از صفر</small></button><button class="action-card" data-action="import"><b>⇧</b><span>ورود Excel</span><small>ادامه فایل قدیمی</small></button><button class="action-card" data-action="open-project"><b>↥</b><span>باز کردن KWM</span><small>ادامه روی هر دستگاه</small></button><button class="action-card" data-action="help"><b>?</b><span>راهنمای سریع</span><small>برای کاربر مبتدی</small></button></div></article><article class="panel dashboard-panel"><div class="panel-head"><h3>یادآوری</h3><span class="badge green">ذخیره محلی</span></div><p class="muted">پس از اولین ذخیره، پروژه روی همین دستگاه قابل بازیابی است. برای انتقال بین دستگاه‌ها از KWM استفاده کن.</p></article></div>`;
    return `<div class="hero dashboard-hero"><div><span class="eyebrow">ACTIVE PROJECT</span><h1>${esc(p.name)}</h1><p>${esc(p.customer||'بدون نام مشتری')} · <span class="badge ${p.status==='تکمیل شده'?'green':'cyan'}">${esc(p.status||'در حال طراحی')}</span></p><div class="hero-actions"><button class="btn primary xl" data-action="continue">▤ ادامه کار</button><button class="btn glass xl" data-action="drawing">⌁ نقشه‌کشی</button><button class="btn glass xl" data-action="backup">☁ بکاپ</button><button class="btn glass xl" data-action="xlsx">⇩ Excel</button></div><div class="command-strip"><span class="muted">آخرین ذخیره: ${new Date(p.updatedAt||Date.now()).toLocaleString('fa-IR')}</span><span class="command-chip">${sheets} شیت</span><span class="command-chip">${drawings} آبجکت</span><span class="command-chip">${parts} قطعه</span><span class="command-chip">${payments} پرداخت</span></div></div><div class="hero-card"><div class="hero-card-top"><span class="badge green">LIVE</span><span class="muted">پروژه فعال</span></div><div class="metric"><strong>${sheets}</strong><span>شیت</span></div><div class="metric"><strong>${drawings}</strong><span>آبجکت نقشه</span></div><div class="metric"><strong>${parts}</strong><span>قطعه</span></div><div class="metric"><strong>${payments}</strong><span>پرداخت</span></div></div></div><div class="dashboard-grid"><article class="panel dashboard-panel"><div class="panel-head"><div><span class="eyebrow">WORKSPACE</span><h2>پرکاربردترین بخش‌ها</h2></div><span class="badge purple">سریع</span></div><div class="quick-actions dashboard-actions"><button class="action-card" data-action="continue"><b>▤</b><span>Workbook</span><small>ویرایش شیت‌ها</small></button><button class="action-card" data-action="drawing"><b>⌁</b><span>Drawing Studio</span><small>نقشه واقعی</small></button><button class="action-card" data-action="parts"><b>◫</b><span>قطعات و برش</span><small>لیست قطعات</small></button><button class="action-card" data-action="backup"><b>☁</b><span>ذخیره امن</span><small>Telegram / KWM</small></button></div></article><article class="panel dashboard-panel"><div class="panel-head"><div><span class="eyebrow">EASY GUIDE</span><h2>راهنمای سریع</h2></div></div><div class="helper-list"><div class="helper-row"><i>۱</i><div><b>کار را از «ادامه کار» شروع کن</b><span>شیت یا نقشه موردنظر را باز کن.</span></div></div><div class="helper-row"><i>۲</i><div><b>در نقشه، اندازه واقعی وارد کن</b><span>واحد را انتخاب کن و با Snap رسم کن.</span></div></div><div class="helper-row"><i>۳</i><div><b>قبل از خروج، بکاپ بگیر</b><span>KWM یا Telegram Backup بساز.</span></div></div></div></article></div>${recent.length?`<article class="panel dashboard-panel" style="margin-top:16px"><div class="panel-head"><div><span class="eyebrow">RECENT</span><h2>پروژه‌های اخیر</h2></div><button class="btn" data-view="projects">همه پروژه‌ها</button></div><div class="recent-projects">${recent.map(r=>`<div class="recent-card"><b>${esc(r.name)}</b><small>${esc(r.customer||'بدون مشتری')} · ${new Date(r.updatedAt||Date.now()).toLocaleDateString('fa-IR')}</small><div style="margin-top:8px"><button class="btn" data-open-project="${r.id}">باز کردن</button></div></div>`).join('')}</div></article>`:''}`;
  }
  ...
  document.addEventListener('click',e=>{const v=e.target.closest?.('[data-view]')?.dataset?.view;if(v)setView(v);});
  ...
  async function telegramBackup(){
    if(!state.project)return;
    state.worker.status='uploading';state.worker.message='در حال ساخت فایل‌های بکاپ...';render();
    const base=C.workerUrl.replace(/\/$/,'');
    try{
      const [xlsx,pdf]=await Promise.all([exportXlsxBlob(),buildPrintPdf('project')]);
      const ts=stamp(),baseName=safeName(state.project.name);
      const fd=new FormData();
      fd.append('project',projectBlob(),`${baseName}_${ts}.kwm`);
      fd.append('xlsx',new Blob([xlsx],{type:'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet'}),`${baseName}_${ts}.xlsx`);
      fd.append('pdf',pdf,`${baseName}_${ts}_report.pdf`);
      fd.append('projectName',state.project.name);fd.append('projectId',state.project.id);fd.append('clientTime',nowISO());fd.append('appVersion',C.version);
      const ctl=new AbortController();const timer=setTimeout(()=>ctl.abort(),45000);
      const r=await fetch(base+C.workerBackupPath,{method:'POST',body:fd,signal:ctl.signal});clearTimeout(timer);
      const d=await r.json().catch(()=>({}));
      if(!r.ok||!d.ok){const detail=(d.files||[]).filter(x=>!x.telegram).map(x=>`${x.name}: ${x.error}`).join(' | ');throw new Error([d.error,detail].filter(Boolean).join(' — ')||`Worker پاسخ ${r.status}`);}
      const count=d.files?.filter(x=>x.telegram).length||0;state.worker.status='ok';state.worker.message=`${count} فایل به Telegram ارسال شد`;render();showToast(`بکاپ موفق: ${count} فایل`,'success');
    }catch(e){state.worker.status='error';state.worker.message=e.name==='AbortError'?'آپلود بیش از حد طول کشید؛ اندازه فایل‌ها و Worker را بررسی کن':(e.message||String(e));render();showToast(state.worker.message,'danger');}
  }
  ...
  async function telegramTest(){
    state.worker.status='testing';state.worker.message='در حال تست Bot + Chat ID...';render();
    const base=C.workerUrl.replace(/\/$/,'');
    const ctl=new AbortController();const timer=setTimeout(()=>ctl.abort(),15000);
    try{const r=await fetch(base+C.workerTestPath,{method:'POST',headers:{'Accept':'application/json'},signal:ctl.signal});const d=await r.json().catch(()=>({}));if(!r.ok||!d.ok){const parts=[d.error,d.telegram?.description,d.chat?.description].filter(Boolean);throw new Error(parts.join(' — ')||`Telegram ${r.status}`);}state.worker.status='ok';state.worker.message=`ارسال تست موفق؛ @${d.bot?.username||'Bot'} → ${d.chat?.title||d.chat?.id||'chat'}`;showToast('پیام تست Telegram ارسال شد','success');}
    catch(e){state.worker.status='error';state.worker.message=e.name==='AbortError'?'Timeout در ارتباط با Worker':(e.message||String(e));showToast(state.worker.message,'danger');}
  ...
  async function boot(){try{await DB.open();const arr=await DB.all('projects');state.projects=arr.filter(p=>p && !p.__snapshotOnly);state.project=state.project||null;if(arr.length){state.project=arr.sort((a,b)=>new Date(b.updatedAt)-new Date(a.updatedAt))[0];state.view='dashboard';}render();if(state.project)syncPartsSheet();}catch(e){render();showToast(`Storage error: ${e.message}`,'danger');}}
  boot();
})();
